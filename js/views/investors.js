...
import { fmtU, fmtKU, amtCell, esc, aggInvestors } from '../utils.js';
...
function retOf(i){
  ...
  if(!p) return { gain:0, total:(i.usd||0), known:false };
  const g = (p.salePrice||0) - (p.budget||0);
  const gain = g * (i.pct||0) / 100;
  return { gain, total:(i.usd||0) + gain, known:true };
}

export function rInversores(){
  const list = aggInvestors();
  const total = D.investments.reduce((a,i)=>a+(i.usd||0),0);
  ...
  <div class="content">
    <div class="kpi-grid">
      ...
      <div class="kpi"><i class="ti ti-cash"></i><div class="kpi-lbl">Capital total</div><div class="kpi-val">${fmtKU(total)}</div></div>
      <div class="kpi"><i class="ti ti-trending-up"></i><div class="kpi-lbl">Retorno estimado total</div><div class="kpi-val gr">${fmtKU(totRet)}</div></div>
    ...
    <div class="card"><div class="card-head"><div class="card-title">Inversores registrados</div><span style="font-size:12px;color:var(--text3)">Retorno = aporte + % de la ganancia estimada del proyecto</span></div>
      ...
      <tbody>${list.length?list.map(x=>`<tr class="clickable" data-oinv="${encodeURIComponent(x.name)}"><td style="font-weight:500">${esc(x.name)}</td><td style="color:var(--text2)">${[...x.projects].map(esc).join(', ')}</td><td>${x.count}</td><td style="color:var(--orange);font-weight:500">${fmtU(x.total)}</td><td style="color:var(--green);font-weight:600">${fmtU(retByName(x.name))}</td></tr>`).join(''):`<tr><td colspan="5" style="text-align:center;color:var(--text3);padding:2rem">Sin inversores aún. Registrá el primer aporte.</td></tr>`}</tbody></table>
    </div>
  </div>`;
}
...
export function rInvD(){
  ...
  const total = items.reduce((a,i)=>a+(i.usd||0),0);
  ...
  <div class="topbar">
    ...
    <div><h1>${esc(name.toUpperCase())}</h1><div class="topbar-sub">${items.length} aporte${items.length!==1?'s':''} · ${fmtU(total)} aportado</div></div>
  ...
  <div class="content">
    <div class="kpi-grid">
      <div class="kpi"><i class="ti ti-cash"></i><div class="kpi-lbl">Total aportado</div><div class="kpi-val">${fmtKU(total)}</div></div>
      <div class="kpi"><i class="ti ti-chart-arrows-vertical"></i><div class="kpi-lbl">Ganancia estimada</div><div class="kpi-val gr">${fmtKU(gain)}</div></div>
      <div class="kpi"><i class="ti ti-coin"></i><div class="kpi-lbl">Total a recibir est.</div><div class="kpi-val gr">${fmtKU(total + gain)}</div></div>
    ...
    <div class="card"><div class="card-head"><div class="card-title">Aportes y proyección</div><span style="font-size:12px;color:var(--text3)">Estimado según precio de venta y presupuesto de cada proyecto</span></div>
      ...
      <tbody>${items.map(i=>{ const r = retOf(i); return `<tr><td>${esc(i.project)}${!r.known?' <span class="badge ba" title="El proyecto no existe o fue renombrado">sin proyecto</span>':''}</td><td style="font-weight:500;color:var(--orange)">${amtCell(i)}</td><td><span class="badge bb">${i.pct||0}%</span></td><td style="color:var(--green)">${fmtU(r.gain)}</td><td style="font-weight:600;color:var(--green)">${fmtU(r.total)}</td><td>${i.date||''}</td><td style="white-space:nowrap"><button class="link-btn" data-iedit="${i.id}" title="Editar"><i class="ti ti-edit"></i></button> <button class="tbl-del" data-idel="${i.id}"><i class="ti ti-trash"></i></button></td></tr>`; }).join('')}</tbody></table></div>

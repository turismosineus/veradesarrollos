...
export function rFinanzas(){
  const rows = D.projects.map(p => {
    ...
    const capital = D.investments.filter(i => i.projectId === p.id).reduce((a,i)=>a+(i.usd||0),0);
  ...
  <div class="content">
    <div class="kpi-grid">
      ...
      <div class="kpi"><i class="ti ti-receipt"></i><div class="kpi-lbl">Gasto registrado</div><div class="kpi-val">${fmtKU(T('registrado'))}</div></div>
      <div class="kpi"><i class="ti ti-home-dollar"></i><div class="kpi-lbl">Venta estimada</div><div class="kpi-val">${fmtKU(tVenta)}</div></div>
      <div class="kpi"><i class="ti ti-trending-up"></i><div class="kpi-lbl">Ganancia estimada</div><div class="kpi-val" style="color:${T('ganancia')>=0?'var(--green)':'var(--red)'}">${fmtKU(T('ganancia'))}</div></div>
    </div>
    <div class="card">
      ...
      <tbody>${rows.length ? rows.map(r=>`<tr class="clickable" data-openfin="${r.p.id}">
        <td style="font-weight:500">${esc(r.p.name)}<div style="font-size:11px;color:var(--text3)">Capital: ${fmtU(r.capital)}</div></td>
        <td>${sBadge(r.p.status)}</td>
        <td>${fmtU(r.p.budget)}</td>
        <td style="color:var(--purple);font-weight:500">${r.proyectado?fmtU(r.proyectado):'<span style="color:var(--text3)">sin elegir</span>'}</td>
        <td style="color:var(--orange);font-weight:500">${fmtU(r.registrado)}</td>
        <td>${r.ejec!=null?`<span class="badge ${r.ejec>100?'br':(r.ejec>=80?'ba':'bg')}">${r.ejec}%</span>`:'—'}</td>
        <td>${fmtU(r.p.salePrice)}</td>
        <td style="font-weight:600;color:${r.ganancia>=0?'var(--green)':'var(--red)'}">${fmtU(r.ganancia)} <span style="font-size:11px;color:var(--text3);font-weight:400">(${r.margen}%)</span></td>
      </tr>`).join('') : `<tr><td colspan="8" style="text-align:center;color:var(--text3);padding:2rem">Sin proyectos todavía.</td></tr>`}</tbody>
      ${rows.length?`<tfoot><tr><td style="padding:10px 1.25rem;font-weight:600;color:var(--text2)">TOTAL</td><td></td><td style="font-weight:600">${fmtU(tPres)}</td><td style="font-weight:600;color:var(--purple)">${fmtU(T('proyectado'))}</td><td style="font-weight:600;color:var(--orange)">${fmtU(T('registrado'))}</td><td></td><td style="font-weight:600">${fmtU(tVenta)}</td><td style="font-weight:700;color:${T('ganancia')>=0?'var(--green)':'var(--red)'}">${fmtU(T('ganancia'))}</td></tr></tfoot>`:''}
      </table>
    </div>

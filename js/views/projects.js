...
// Pestaña "Finanzas" del proyecto (todo en USD; los montos en pesos muestran su equivalente).
function rFinanzasProy(p, dir){
  ...
  const invTotal = projInvest.reduce((a,i)=>a+(i.usd||0),0);
  ...
  const panel = `${refBar}${warn}
    ...
    <div class="two-col">
      <div class="sec"><h3>Resumen financiero <span style="font-size:11px;color:var(--text3);font-weight:400">en USD</span></h3>
        <div class="stat-row"><span class="sl">Capital invertido</span><span class="sv">${fmtU(invTotal)}</span></div>
        <div class="stat-row"><span class="sl">Gasto registrado (real)</span><span class="sv">${fmtU(tReg)}</span></div>
        ${dir?`<div class="stat-row"><span class="sl">Gasto proyectado (elegidos)</span><span class="sv or">${fmtU(projected)}</span></div>`:''}
        <div class="stat-row"><span class="sl">Presupuesto total</span><span class="sv">${fmtU(p.budget)}</span></div>
        <div class="stat-row"><span class="sl">Precio de venta est.</span><span class="sv">${fmtU(p.salePrice)}</span></div>
        <div class="stat-row"><span class="sl">Ganancia proyectada</span><span class="sv gr">${fmtU(g)} (${pct}%)</span></div></div>
      <div class="sec"><h3>Distribución financiera</h3><div class="bar-chart">
        ${[['Registrado',tReg,'#f97316'],['Proyectado',projected,'#a855f7'],['Presupuesto',p.budget,'#f59e0b'],['Venta est.',p.salePrice,'#22c55e']].map(([l,vv,c])=>`<div class="bar-wrap"><div class="bar-val">${fmtKU(vv)}</div><div class="bar" style="height:${Math.round((vv/Math.max(tReg,projected,p.budget,p.salePrice,1))*80)+10}px;background:${c}"></div><div class="bar-lbl">${l}</div></div>`).join('')}
      </div></div>
    </div>`;
  ...
  const pendRows = concepts.map(c => {
    ...
    return rows.map((b,i)=>`<tr><td style="font-weight:500">${i===0?esc(c):''}${i===0&&already?' <span class="badge bgr" title="Ya hay un presupuesto elegido para este concepto">ya elegido</span>':''}</td><td>${esc(b.provider)}</td><td style="font-weight:600;color:${est(b)!=null&&est(b)===min&&rows.length>1?'var(--green)':'var(--text)'}">${amtCell(b, rr, true)}${est(b)!=null&&est(b)===min&&rows.length>1?' <span class="badge bg">más bajo</span>':''}</td><td>${bb(b.status)}</td><td>${b.date||''}</td><td>${fileBtns(b, b.name||'presupuesto')}</td>${dir?`<td style="white-space:nowrap"><button class="btn-or sm" data-bdchoose="${b.id}"><i class="ti ti-check"></i> Elegir</button> ${b.status!=='rechazado'?`<button class="link-btn" data-bdreject="${b.id}" title="Rechazar" style="color:var(--red)"><i class="ti ti-x"></i></button>`:''}</td>`:''}</tr>`).join('');
  }).join('');
  ...
  const gastosSec = `${warn}
    <div class="kpi-grid">
      <div class="kpi"><i class="ti ti-receipt"></i><div class="kpi-lbl">Gastos (compras)</div><div class="kpi-val">${fmtKU(tG)}</div></div>
      <div class="kpi"><i class="ti ti-tool"></i><div class="kpi-lbl">Liquidaciones</div><div class="kpi-val">${fmtKU(tL)}</div></div>
      <div class="kpi"><i class="ti ti-sum"></i><div class="kpi-lbl">Total registrado</div><div class="kpi-val">${fmtKU(tReg)}</div></div>
...
function rInversoresProy(p, dir){
  ...
  return `
    <div class="kpi-grid">
      <div class="kpi"><i class="ti ti-cash"></i><div class="kpi-lbl">Capital invertido</div><div class="kpi-val">${fmtKU(capital)}</div></div>
      ...
      <div class="kpi"><i class="ti ti-trending-up"></i><div class="kpi-lbl">Ganancia a repartir (est.)</div><div class="kpi-val gr">${fmtKU(gain)}</div></div>
    </div>
    <div class="sec" style="padding:.9rem 1.25rem">
      <div style="display:flex;justify-content:space-between;font-size:12px;color:var(--text2);margin-bottom:6px"><span>Capital reunido vs presupuesto del proyecto</span><span>${fmtU(capital)} / ${fmtU(p.budget)}</span></div>
    ...
    <div class="card">
      ...
      <tbody>${items.map(i=>`<tr><td style="font-weight:500">${esc(i.investor)}</td><td style="font-weight:500;color:var(--orange)">${amtCell(i)}</td><td><span class="badge bb">${i.pct||0}%</span></td><td style="color:var(--green)">${fmtU(ret(i))}</td><td style="font-weight:600;color:var(--green)">${fmtU((i.usd||0) + ret(i))}</td><td>${i.date||''}</td><td style="color:var(--text2)">${esc(i.note||'-')}</td>${dir?`<td style="white-space:nowrap"><button class="link-btn" data-iedit="${i.id}" title="Editar"><i class="ti ti-edit"></i></button> <button class="tbl-del" data-idel="${i.id}"><i class="ti ti-trash"></i></button></td>`:''}</tr>`).join('')}</tbody>
      <tfoot><tr><td style="padding:10px 1.25rem;font-weight:600;color:var(--text2)">TOTAL</td><td style="font-weight:700;color:var(--orange)">${fmtU(capital)}</td><td><span class="badge bb">${pctTotal}%</span></td><td style="color:var(--green);font-weight:600">${fmtU(items.reduce((a,i)=>a+ret(i),0))}</td><td style="color:var(--green);font-weight:700">${fmtU(capital + items.reduce((a,i)=>a+ret(i),0))}</td><td colspan="${dir?3:2}"></td></tr></tfoot></table>`
      : `<div style="text-align:center;padding:2rem;color:var(--text3)">Sin inversores en este proyecto. Registrá el primer aporte.</div>`}
    </div>
    <div style="font-size:11px;color:var(--text3);padding:0 4px">Todo en USD. Retorno estimado = participación % × (precio de venta estimado − presupuesto).</div>`;
}

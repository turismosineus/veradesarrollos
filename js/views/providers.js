...
import { esc, fmt, fmtK, fmtU, amtCell, refRate, oTotal } from '../utils.js';
  ...
  if(tab==='info') body = `
    ...
    body = `<div class="sec"><h3><span>Presupuestos</span><button class="btn-or sm" id="new-budget"><i class="ti ti-upload"></i> Cargar presupuesto</button></h3>
      ...
      :`<table><thead><tr><th>Concepto</th><th>Proyecto</th><th>Monto</th><th>Estado</th><th>Fecha</th><th>Archivo</th><th></th></tr></thead><tbody>
        ${buds.map(b=>`<tr><td style="font-weight:500">${esc(b.concept||b.name||'-')}${b.note?`<div style="font-size:11px;color:var(--text3)">${esc(b.note)}</div>`:''}</td><td style="color:var(--text2)">${esc(b.project||'-')}</td><td style="font-weight:600;color:var(--orange)">${amtCell(b, refRate(), true)}</td><td>${budgetBadge(b.status)}</td><td>${b.date||''}</td>

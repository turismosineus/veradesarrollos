...
export const amtCell = (x, fb, quiet) => fmtAmt(x.amount, x.currency) + (x.currency === 'USD' ? '' : (x.usd != null ? '<div style="font-size:11px;color:var(--text3)">≈ ' + fmtU(x.usd) + ' @ ' + x.rate + '</div>' : (fb ? '<div style="font-size:11px;color:var(--text3)">≈ ' + fmtU((x.amount||0)/fb) + ' <span title="al último dólar cargado, solo orientativo">(≈ a ' + fb + ')</span></div>' : (quiet ? '' : '<div style="font-size:11px;color:var(--amber)">sin cotización</div>'))));

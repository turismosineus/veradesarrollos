...
export function rFinanzas(){
  ...
  if(tab === 'liquidaciones'){
    body = `<div class="card"><div class="card-head"><div class="card-title">Liquidaciones — mano de obra</div><button class="btn-or sm" id="new-liq"><i class="ti ti-plus"></i> Registrar liquidación</button></div>
      ...
      <tbody>${(D.liquidaciones||[]).length ? (D.liquidaciones||[]).map(l=>`<tr><td style="font-weight:500">${esc(l.worker)}</td><td><span class="badge bb">${esc(l.trade||'-')}</span></td><td style="color:var(--text2)">${esc(l.project||'-')}</td><td style="font-weight:500">${fmt(l.amount)}</td><td>${l.date||''}</td><td style="color:var(--text2)">${esc(l.note||'-')}</td><td style="white-space:nowrap"><button class="link-btn" data-ledit="${l.id}" title="Editar"><i class="ti ti-edit"></i></button> <button class="tbl-del" data-ldel="${l.id}"><i class="ti ti-trash"></i></button></td></tr>`).join('') : empty(7,'Sin liquidaciones cargadas. Registrá acá los pagos de mano de obra.')}</tbody></table></div>`;
  } else if(tab === 'gastos'){
